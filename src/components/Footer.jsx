          ...
          <div>
            <h2 className="text-5xl md:text-8xl font-bold uppercase tracking-tighter leading-none mb-12">
              ...
              <span className="text-primary">
                {s.site_title ? `${s.site_title}`.split(" ")[0] : "INITIATE"}
              </span>
            </h2>
            <div className="grid grid-cols-2 gap-8 md:gap-12">
              <div>
                ...
                <p className="text-sm md:text-base font-bold">
                  {s.site_email || "connect@crabstack.io"}
                </p>
                {s.site_phone && (
                  <p className="text-sm md:text-base font-bold mt-2">
                    {s.site_phone.split(",").map((n, i) => (
                      <span key={i}>
                        {n.trim()}
                        <br />
                      </span>
              ...
              <div>
                ...
                {s.site_address ? (
                  s.site_address.split(",").map((line, i) => (
                    <p key={i} className="text-sm md:text-base font-bold">
                      {line.trim()}
                    </p>
                  ))
                ) : (
          ...
          <div className="flex flex-col justify-between">
            <div className="max-w-md">
              <p className="text-white/60 text-base md:text-lg font-light leading-relaxed mb-12">
                {"Join the vanguard of digital engineering. Now accepting high-priority digital missions."}
              </p>
              ...
              {status === "success" && (
                <p className="text-green-400 text-[10px] tracking-widest uppercase mt-2">
                  Subscribed successfully
                </p>
              )}
              {status === "error" && (
                <p className="text-red-400 text-[10px] tracking-widest uppercase mt-2">
                  Failed — try again
                </p>
              )}
            </div>
            ...
            <div className="flex items-center gap-6 md:gap-12 mt-16 md:mt-24 flex-wrap">
              ...
              <div className="flex gap-6 md:gap-8">
                {s.social_twitter || s.social_instagram || s.social_linkedin ? (
                  <>
                    {s.social_twitter && (
                      <a
                        ...
                        className="text-[10px] text-white/60 hover:text-primary transition-colors font-bold tracking-widest min-w-[28px] min-h-[28px] flex items-center justify-center"
                      >
                        TW
                      </a>
                    )}
                  ...
                  <>
                    <span className="text-[10px] text-white/50 font-bold tracking-widest">
                      TW
                    </span>
                    <span className="text-[10px] text-white/50 font-bold tracking-widest">
                      IG
